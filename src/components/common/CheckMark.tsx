...
export function GroupCheckMark({ msgId, lastReadMsgId, allMsgIds, members, senderUid }: GroupCheckMarkProps) {
  const otherMembers = members.filter(uid => uid !== senderUid);
  if (otherMembers.length === 0) return null;
  const msgIndex = allMsgIds.indexOf(msgId);
  const allRead = msgIndex !== -1 && otherMembers.every(uid => {
